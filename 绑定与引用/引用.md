...
- 堆栈非常快，但是堆却没有那么快。
- 堆栈需要在编译时知道变量的大小。因此，像 i32 这样的简单变量就会进入堆栈，因为知道它们的确切大小。
- 有些类型在编译时不知道大小。但是堆栈需要知道确切的大小。你是做什么？您将数据放入堆中，因为堆可以具有任何大小的数据。为了找到它，将指针放在堆栈上，因为一直都知道指针的大小。
...
Reference 在 Rust 中非常重要。Rust 使用引用来确保所有内存访问都是安全的。使用 ＆ 来创建 Reference：
...
`country` 是 String 类型，创建了指向它的两个引用，它们的类型就是 &String，还可以根据需要创建无数个这样的引用。
...
函数 `return_str()` 创建一个字符串，然后创建对该字符串的引用。然后，它尝试返回引用。但是 `country` 只存在于函数内部。因此，函数结束后，`country_ref` 指的是已经消失的内存。Rust 可以防止在内存方面犯错误。

# 反引用与点运算符

当您有参考时，需要使用 `*` 来获取值。引用是另一种类型，因此无法使用：
...
将第 5 行更改为 `println!("{}", my_number == *reference);` 就可以了，这也就是所谓的 Dereferencing。但是，当您使用一种方法时，Rust 将为您取消引用，函数中的 `.` 称为点运算符。
...
为了使其工作，需要取消引用：`println!("{}", *reference_number == 8);`，但是使用点运算符，不需要 `*`。例如：
...
为 Item 创建一个将数字与另一个数字进行比较的方法。不需要在任何地方使用 `*`：
...
将其添加到 `my_number` 中使用 10。但是您不能编写 `num_ref + = 10`，因为 `num_ref` 不是 i32 值。为了到达值所在的地方，使用 `*`。`*`表示我不需要引用，我想要引用后面的值。换句话说，一个 `*` 会删除一个＆。
...
编译器使用 `number_change` 来更改数字，但是没有再次使用它，即不会同时使用不可变和可变的引用。
...
- Step 1: 创建 `String` `country`. `country` 是所有者。
- Step 2: 给予 `country` 至 `print_country`. `print_country` 没有一个 `->`, 因此它不会返回任何内容。后 `print_country` 完成， `String` 现在已经死了。
- Step 3: 尝试给 `country` 至 `print_country`，但已经做到了。没有 `country` 给予。
...
当然，如果 String 很大，.clone() 会占用大量内存。例如，一个 String 可以是整本书，每当调用 .clone() 时，它将复制该书。因此，如果可以的话，使用 ＆ 作为引用会更快。
